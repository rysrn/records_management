...
Class Master extends DBConnection {
	...
	function save_offender(){
		foreach($_POST as $k =>$v){
			$_POST[$k] = addslashes($v);
		}
		extract($_POST);
		$name = ucwords($lastname.', '.$firstname.' '.$middlename);
		$chk = $this->conn->query("SELECT * FROM `offenders_list` where  ID_no = '{$ID_no}' ".($id>0? " and id!= '{$id}' " : ""))->num_rows;
		$this->capture_err();
		if($chk > 0){
			$resp['status'] = 'failed';
			$resp['msg'] = "ID no already exist in the database. Please review and try again.";
			return json_encode($resp);
			exit;
		}
		if(empty($id))
			$sql1 = "INSERT INTO `offenders_list` set `name` = '{$name}', ID_no = '{$ID_no}' ";
		else
			$sql1 = "UPDATE `offenders_list` set `name` = '{$name}', ID_no = '{$ID_no}' where id = '{$id}' ";
		
		$save1 = $this->conn->query($sql1);
		$this->capture_err();
		$offender_id = empty($id) ? $this->conn->insert_id : $id ;
		$this->conn->query("DELETE FROM `offenders_meta` where offender_id = '{$offender_id}' ");
		$data = "";
		foreach($_POST as $k =>$v){
			if(!in_array($k,array('id'))){
				if(!empty($data)) $data .=",";
				$v = addslashes($v);
				$data .= " ('{$offender_id}','{$k}','{$v}') ";
			}
		}
		$data .= ",('{$offender_id}','offender_id','{$offender_id}')";

		
		$sql = "INSERT INTO `offenders_meta` (`offender_id`,`meta_field`,`meta_value`) VALUES {$data} ";
		$save = $this->conn->query($sql);
		$this->capture_err();
		if($save){
			$resp['status'] = 'success';
			if(empty($id))
				$this->settings->set_flashdata('success',"New Offender successfully saved.");
			else
				$this->settings->set_flashdata('success',"Offender Details successfully updated.");
			$id = empty($id) ? $this->conn->insert_id : $id;
			$dir = 'uploads/offenders/';
			if(!is_dir(base_app.$dir))
				mkdir(base_app.$dir);
			if(isset($_FILES['img'])){
				if(!empty($_FILES['img']['tmp_name'])){
					$fname = $dir.$offender_id.".".(pathinfo($_FILES['img']['name'], PATHINFO_EXTENSION));
					$move =  move_uploaded_file($_FILES['img']['tmp_name'],base_app.$fname);
					if($move){
						$this->conn->query("INSERT INTO `offenders_meta` set `meta_value` = '{$fname}', offender_id = '{$offender_id}',`meta_field` = 'image_path' ");
	...
	function delete_offender(){
		extract($_POST);
		$qry = $this->conn->query("SELECT * FROM `offenders_meta` where offender_id = '{$id}'");
		while($row=$qry->fetch_assoc()){
			${$row['meta_field']} = $row['meta_value'];
		}
		$del = $this->conn->query("DELETE FROM `offenders_list` where id = '{$id}'");
		$this->capture_err();
		if($del){
			$resp['status'] = 'success';
			if(is_file(base_app.$image_path))
				unlink((base_app.$image_path));
			$this->settings->set_flashdata('success',"Offender's Info successfully deleted.");
		}else{
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
		}
	...
	function save_offense_record(){
		...
		$save = $this->conn->query($sql);
		$this->capture_err();
		$offender_offense_id = empty($id) ? $this->conn->insert_id : $id;
		$this->conn->query("DELETE FROM `offense_items` where `offender_offense_id` = '{$offender_offense_id}'");
		$this->capture_err();
		$data = "";
		foreach($offense_id as $k => $v){
			if(!empty($data)) $data .= ", ";
			$data .= "('{$offender_offense_id}','{$v}','{$fine[$k]}','{$status}','{$date_created}')";
		}
		$save2= $this->conn->query("INSERT INTO `offense_items` (`offender_offense_id`,`offense_id`,`fine`,`status`,`date_created`) VALUES {$data}");
		$this->capture_err();
		if($save && $save2){
			if(empty($id))
...
switch ($action) {
	...
	break;
	case 'save_offender':
		echo $Master->save_offender();
	break;
	case 'delete_offender':
		echo $Master->delete_offender();
	break;
