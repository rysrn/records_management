...
if(isset($_GET['id']) && $_GET['id'] > 0){
    $qry = $conn->query("SELECT r.*,d.ID_no, d.name as offender from `offense_list` r inner join `offenders_list` d on r.offender_id = d.id where r.id = '{$_GET['id']}' ");
    if($conn->error){
        echo $conn->error ."\n";
        echo "SELECT r.*,d.ID_no, d.name as offender from `offense_list` r inner join `offender_list` on r.offender_id = d.id where r.id = '{$_GET['id']}' ";
    }
    $qry2 = $conn->query("SELECT i.*,o.code,o.name from `offense_items` i inner join `offenses` o on i.offense_id = o.id where i.offender_offense_id = '{$_GET['id']}' ");
    if($qry->num_rows > 0){
                    ...
                    <div class="row">
                        <div class="col-12">
                            ...
                            <div class="d-flex w-max-100">
                                <label class="float-left w-auto whitespace-nowrap">ID no:</label>
                                <p class="col-md-auto border-bottom border-dark w-100"><b><?php echo $ID_no ?></b></p>
                            </div>
                            <div class="d-flex w-max-100">
                                <label class="float-left w-auto whitespace-nowrap">offender's Name:</label>
                                <p class="col-md-auto border-bottom border-dark w-100"><b><?php echo $offender ?></b></p>
                            </div>
                        </div>
                    </div>
                </td>
...
<script>
    $(function(){
        $('#print').click(function(){
            ...
            '<h5 class="text-center">Offense Ticket</h5>'+
            '</div>'+
            '</div><hr/>');
            _el.append(_h)
