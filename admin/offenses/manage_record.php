	...
	<div class="card-body">
        ...
        <div class="row">
            <div class="col-6">
                ...
                <div class="form-group">
                    <lable class="control-label" for="offender_id">Offender</lable>
                    <select name="offender_id" id="offender_id" class="custom-select select2" required>
                        <option value=""></option>
                        <?php 
                        $offender = $conn->query("SELECT * FROM `offenders_list` order by `name` asc ");
                        while($row = $offender->fetch_assoc()):
                        ?>
                        <option value="<?php echo $row['id'] ?>" <?php echo (isset($offender_id) && $offender_id == $row['id']) ? 'selected' : '' ?>>[<?php echo $row['ID_no'] ?>] <?php echo ucwords($row['name']) ?></option>
                        <?php endwhile; ?>
                    </select>
                </div>
            </div>
            ...
            <div class="col-6">
                    ...
                    <div class="col-7">
                        <div class="form-group">
                            <select id="offense_id" class="custom-select select2" >
                                <option value=""></option>
                                <?php 
                                $offender = $conn->query("SELECT * FROM `offenses` order by `name` asc ");
                                while($row = $offender->fetch_assoc()):
                                ?>
                ...
                <table class="table table-stripped table-hover" id="fine-list">
                    ...
                    <tbody>
                        <?php
                        if(isset($id)):
                        $olist = $conn->query("SELECT i.*,o.code,o.name FROM `offense_items` i inner join `offenses` o on i.offense_id = o.id where i.offender_offense_id ='{$id}' ");
			...
			$.ajax({
				...
				success:function(resp){
					if(typeof resp =='object' && resp.status == 'success'){
                        end_loader();
                        uni_modal("<i class='fa fa-ticket'></i> Offender's Offense Ticket Details","offenses/view_details.php?id="+resp.id,'mid-large')
                        setTimeout(() => {
                            end_loader();
                        }, 500);
