...
<div class="container-fluid">
    ...
    <div class="border border-dark px-2 py-2" id="print_out">
            ...
            <tr class='boder-0'>
                ...
                <td width="20%">
                    <div class="w-100 d-flex align-items-center justify-content-center">
                        <img src="<?php echo validate_image($image_path) ?>" alt="Offender's ID" class="img-thumnail" id="cimg">
                    </div>
                </td>
            </tr>
        </table>
        ...
        <table class='table table-stripped px-4'>
            ...
            <tbody>
                <?php 
                $olist = $conn->query("SELECT i.*,o.code,o.name FROM `offense_items` i inner join `offenses` o on i.offense_id = o.id where i.offender_offense_id in (SELECT id FROM `offense_list` where offender_id = '{$id}') order by unix_timestamp(i.date_created) asc  ");
                if($conn->error){
                    echo $conn->error ."\n";
                }
...
<script>
    $(function(){
        $('#print').click(function(){
            ...
            '<h3 class="text-center">Offender\'s Information and Offense Records</h3>'+
            '</div>'+
            '</div><hr/>');
            _el.append(_p)
